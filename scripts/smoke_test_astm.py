...
ASTM_SMOKE_DB = ROOT / "data" / "smoke_test_astm.sqlite3"
...
def main() -> None:
    ASTM_SMOKE_DB.parent.mkdir(parents=True, exist_ok=True)
    if ASTM_SMOKE_DB.exists():
        ASTM_SMOKE_DB.unlink()

    os.environ["LIS_DATABASE_URL"] = f"sqlite:///{ASTM_SMOKE_DB.as_posix()}"
    app = create_app()
