...
FHIR_SMOKE_DB = ROOT / "data" / "smoke_test_fhir.sqlite3"
...
def main() -> None:
    FHIR_SMOKE_DB.parent.mkdir(parents=True, exist_ok=True)
    if FHIR_SMOKE_DB.exists():
        FHIR_SMOKE_DB.unlink()

    os.environ["LIS_DATABASE_URL"] = f"sqlite:///{FHIR_SMOKE_DB.as_posix()}"
    app = create_app()
